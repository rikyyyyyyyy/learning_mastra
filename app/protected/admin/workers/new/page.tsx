...
export default function NewWorkerPage() {
  ...
  const [form, setForm] = useState({
    ...
    prompt_text: '',
    enabled: true,
    metadata: { mcp: { exa: { enabled: false }, custom: [] as CustomMCP[] } } as Metadata & { mcp: { custom: CustomMCP[] } },
  });
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setBasePrompt(base || '');
        // 環境変数の有無はサーバ側でチェックできないため、ベストエフォートでヘッダー経由はしない。UIでは説明のみ表示。
        // 将来: /api/admin/env?key=EXA_API_KEY などで表示しても良い。
      } catch (e) {
        setError(String(e));
      }
    })();
  ...
  async function submit() {
    ...
    setError(null);
    try {
      if (!form.id || !/^[a-z0-9\-_.]+$/i.test(form.id)) throw new Error('IDは英数字・-. _ のみで入力してください');
  ...
  return (
    <div>
      <h2 className="text-xl font-semibold mb-4">ワーカー新規作成</h2>
      {error && <p className="text-red-600 mb-2">{error}</p>}
      <div className="grid gap-3 max-w-xl">
        <label className="grid gap-1">
          <span className="text-sm">ID</span>
          <input className="border rounded p-2" placeholder="worker-1" value={form.id} onChange={e => setForm({ ...form, id: e.target.value })} />
        </label>
        <label className="grid gap-1">
          ...
          <label className="flex items-center gap-2 border rounded px-2 py-1">
            <input
              ...
              onChange={e => setForm(f => ({
                ...f,
                metadata: { mcp: { exa: { enabled: e.target.checked }, custom: (f.metadata.mcp?.custom || []) } },
              }))}
            />
