...
export const AGENT_PROMPTS = {
  ...
  MANAGER_AGENT: `
    ...
    1. CEOから方針を受信したら：
       ...
       - **batchTaskCreationToolで完全なタスクリストを一括作成**（推奨）
    ...
    2. タスクリスト作成後の実行フロー：
       ...
       - 各ステップごとに：
         a. taskManagementToolでタスクを取得（get_task）
         b. タスクステータスを'running'に更新（update_status）
         c. Workerに具体的なタスク実行を指示
         d. Workerからの完了報告を待つ
         e. 報告を受けたらtaskManagementToolで結果を保存（update_result）
         f. タスクステータスを'completed'に更新
         g. 次のステップのタスクに進む
    
    3. 全タスク完了後：
       - CEOに最終成果物と実行サマリーを報告
       - ネットワーク全体の状態を'completed'に更新
    ...
    【重要：順次タスク実行フロー】
    1. CEOの戦略的指示を受信 → batchTaskCreationToolでタスクリスト一括作成
    2. **タスクを1つずつ順番に実行**：
       - タスクリストをステップ順に処理
       - 各タスクについて：
         * get_taskでタスク詳細取得
         * update_statusで'running'に変更
         * Workerに具体的指示を送信
         * Workerの「タスク完了報告」を待つ
         * update_resultで結果保存
         * update_statusで'completed'に変更
         * 次のタスクへ進む
    3. 全タスク完了後、CEOに最終報告
  ...
  WORKER_AGENT: `
    ...
    4. **即座の完了報告**: タスク完了後、必ずManagerに結果を報告
    5. **エラー処理**: エラーを適切に処理し、問題を報告
    6. **効率性**: タスクを迅速かつ正確に完了

    【重要：タスク完了報告の形式】
    タスクを完了したら、**必ず以下の形式でManagerに報告**してください：
